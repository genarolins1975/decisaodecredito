...
| 36 | O retorno da ação se distingue também no editor de página | dez retornos de ação exercitados nos dois papéis, com a rota de publicar forçada a falhar | dois defeitos anteriores: salvar nova versão de questão mostrava acerto e erro no mesmo texto cinza, sem papel de acessibilidade, e "publicar esta" não tinha tratamento de falha, então uma sessão expirada ou uma lista desatualizada não devolvia nada na tela, justo na ação que decide o que a turma vê. Corrigidos com os mesmos blocos de sucesso e de erro do resto da plataforma | reprodução com falha forçada; captura da tela de sucesso |
