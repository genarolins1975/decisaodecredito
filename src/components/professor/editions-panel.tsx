    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
        ...
        {editions.map((e) => (
          <section key={e.id} className="card">
            <div className="flex flex-wrap items-center gap-3 mb-2">
              <h2>Edição {e.label}</h2><StatusBadge status={e.status} />
              <div className="flex-1" />
              {e.status !== "active" && <button className="btn btn-sm btn-secondary" onClick={() => run(async () => { await api(`/api/professor/edicoes/${e.id}/status`, { body: { status: "active" } }); return "Edição ativada."; })}>Ativar</button>}
              {e.status !== "archived" && <button className="btn btn-sm btn-ghost" onClick={() => { if (confirm(`Arquivar a edição ${e.label}? O histórico é mantido e novas interações ficam bloqueadas.`)) run(async () => { await api(`/api/professor/edicoes/${e.id}/status`, { body: { status: "archived" } }); return "Edição arquivada."; }); }}>Arquivar</button>}
            </div>
            <ul className="list-none p-0 m-0 grid gap-2">
              {e.classes.map((c) => (
                <li key={c.id} className="flex flex-wrap items-center gap-3 border border-rule rounded px-3 py-2 bg-paper">
                  ...
                  <div className="flex-1" />
                  <button className="btn btn-sm btn-ghost" onClick={() => { const code = prompt("Código da nova turma (ex.: 2027-A):"); if (!code) return; const target = prompt("Rótulo da edição de destino (ex.: 2027):", e.label); const ed = editions.find((x) => x.label === target); if (!ed) { setErr("Edição de destino não encontrada; crie-a antes."); return; } run(async () => { const r = await api<{ warning: string }>(`/api/professor/turmas/${c.id}/duplicar`, { body: { editionId: ed.id, code, name: `Turma ${target}` } }); return r.warning; }); }}>Duplicar turma</button>
            ...
            <form className="mt-3 flex flex-wrap gap-2 items-end" onSubmit={(ev) => { ev.preventDefault(); const fd = new FormData(ev.currentTarget); run(async () => { await api("/api/professor/turmas", { body: { editionId: e.id, code: String(fd.get("code")), name: String(fd.get("name") || "") } }); return "Turma criada."; }); }}>
              ...
              <button className="btn btn-sm" type="submit">Nova turma nesta edição</button>
            </form>
          </section>
        ))}
      </div>
      <aside className="flex flex-col gap-4">
        <section className="card">
          <h2 className="text-base mb-2">Nova edição (ano letivo)</h2>
          ...
          <button className="btn btn-sm mt-3" onClick={() => run(async () => { await api("/api/professor/edicoes", { body: { year: Number(year), label: year } }); return `Edição ${year} criada (vazia). Use "Duplicar edição" para copiar o conteúdo.`; })}>Criar edição vazia</button>
        </section>
        <section className="card">
          <h2 className="text-base mb-2">Duplicar edição</h2>
          ...
          <label className="label" htmlFor="df">Origem</label>
          ...
          <label className="label mt-2" htmlFor="dy">Ano da nova edição</label>
          <input id="dy" className="input" value={year} onChange={(e) => setYear(e.target.value)} inputMode="numeric" />
          <button className="btn btn-sm mt-3" onClick={() => run(async () => { const r = await api<{ warning: string; copied: { pages: number; questions: number } }>(`/api/professor/edicoes/${dupFrom}/duplicar`, { body: { year: Number(year), label: year } }); return `${r.copied.pages} páginas e ${r.copied.questions} questões copiadas. ${r.warning}`; })}>Duplicar para {year}</button>
        </section>
      </aside>
    </div>
  );
