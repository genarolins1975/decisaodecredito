...
type Blind = { cfg: { id: string; ootFileId: string | null; labelsFileId: string | null; releasePolicy: string; maxSubmissions: number; feedbackLevel: string; expectedIds: number | null } | null; submissions: { b: { id: string; submissionNo: number; validation: { valid: boolean; missing: number; duplicates: number; extra: number }; metrics: { auc?: number; ks?: number; brier?: number; logloss?: number; n?: number } | null; submittedAt: string }; user: string; group: string | null }[]; freezes: { f: { id: string; modelVersion: string; manifestSha256: string; frozenAt: string; notes: string | null; groupId: string | null; userId: string | null }; group: string | null; user: string | null }[]; datasets?: { id: string; code: string; name: string; ootFileId: string | null; labelsFileId: string | null; groups: number }[] };
    ...
    <div className="flex flex-col gap-5">
      ...
      {assignment.blindTestEnabled && (
        <section className="card" aria-labelledby="cego">
          ...
          <p className="hint mb-3">Com uma base por grupo, o OOT sem desfecho e os rótulos verdadeiros vêm do catálogo de bases da edição (publicados pelo script de dados). Os campos abaixo são o fallback para turmas com base única: valem para os grupos sem base atribuída ou cuja base não tem OOT próprio.</p>
          {blind.datasets && blind.datasets.length > 0 && (
            <ul className="text-[13px] grid gap-1 mb-3 md:grid-cols-3 list-none p-0">
              {blind.datasets.map((d) => <li key={d.id} className="panel-soft py-1 px-2"><span className="font-mono">{d.code}</span> {d.name} · {d.groups} grupo(s) · <span className={d.ootFileId ? "text-ok" : "text-warn"}>{d.ootFileId ? "OOT ✓" : "sem OOT"}</span> · <span className={d.labelsFileId ? "text-ok" : "text-warn"}>{d.labelsFileId ? "rótulos ✓" : "sem rótulos"}</span></li>)}
            </ul>
          )}
