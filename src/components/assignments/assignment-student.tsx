...
type Data = {
  ...
  blind: { configured: boolean; maxSubmissions: number; feedbackLevel: string; freezes: { id: string; modelVersion: string; manifestSha256: string; frozenAt: string; artifactHashes: { name: string; sha256: string }[] }[]; oot: { ok: boolean; reason?: string }; submissions: { id: string; submissionNo: number; validation: { valid: boolean; missing: number; duplicates: number; extra: number; rowsRead: number; expected: number }; submittedAt: string }[] } | null;
};
      ...
      <aside className="flex flex-col gap-4">
        ...
        {a.blindTestEnabled && d.blind && (
          <section className="card" aria-labelledby="cego">
            ...
            <p className="hint mb-2">Ordem obrigatória: congelar o modelo (manifesto, versão e hashes) → receber o arquivo OOT sem desfecho → enviar previsões uma única vez ({d.blind.maxSubmissions} submissão{d.blind.maxSubmissions > 1 ? "ões" : ""}). Os rótulos ficam apenas com o professor.</p>
