...
<title>Visual ${slug}</title><link rel="stylesheet" href="/legado/estilo.css?t=${encodeURIComponent(sig)}"><link rel="stylesheet" href="/legado/katex.css">
...
<script nonce="${nonce}" src="/legado/katex.js"></script>
<script nonce="${nonce}">${bootstrap}</script>
</body></html>`;
  return new NextResponse(html, {
    headers: {
      "content-type": "text/html; charset=utf-8",
      "content-security-policy": `default-src 'none'; script-src 'nonce-${nonce}'; style-src 'self' 'unsafe-inline'; img-src data:; font-src 'self' data:; connect-src 'none'; frame-ancestors 'self'; base-uri 'none'; form-action 'none'`,
