...
import { ABERTURA_NATIVA, PALCO_PROPRIO } from "@/lib/visuais/palco-proprio";
...
export default async function ApresentacaoPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ sessao?: string }> }) {
  ...
  const infografico = pageIndex === 1 && !PALCO_PROPRIO.has(slug) && !ABERTURA_NATIVA.has(slug) ? infograficoDoCapitulo(data.chapter.number) : null;
  return (
    <Slide
