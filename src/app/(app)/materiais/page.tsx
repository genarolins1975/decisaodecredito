...
export default async function MateriaisPage() {
  ...
  // materiais com status "professor" (gabaritos, gerador) nunca chegam ao aluno
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <div>
        ...
        <ul className="list-none p-0 m-0 grid gap-2">
          {materials.map((m) => <li key={m.id} className={`card-flat ${m.status === "professor" ? "border-gold" : ""}`}><p className="eyebrow">{m.kind}{m.status === "professor" ? " · só professor" : ""}</p><p className="font-semibold text-ink text-[15px]">{m.url ? <a href={m.url} target="_blank" rel="noreferrer">{m.title}</a> : m.fileId ? <a href={`/api/arquivos/${m.fileId}`}>{m.title}</a> : m.title}</p>{m.description && <p className="hint mt-1">{m.description}</p>}</li>)}
