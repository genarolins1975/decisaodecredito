...
export default async function AcompanhamentoPage() {
  ...
  const responses = await db.select({ qv: schema.studyResponses.questionVersionId, ok: schema.studyResponses.isCorrect, at: schema.studyResponses.serverTime, attemptNo: schema.studyResponses.attemptNo, disclosedBefore: schema.studyResponses.disclosedBefore, revealedAt: schema.studyResponses.revealedAt }).from(schema.studyResponses).where(and(eq(schema.studyResponses.userId, uid), eq(schema.studyResponses.classId, cid))).orderBy(desc(schema.studyResponses.serverTime));
  ...
  // acerto próprio: a última tentativa está certa e o gabarito não tinha sido divulgado antes dela
