 ...
 */
import { BETA0, BETA1, BETA2, arredondar, fmt, fmtPct, fmtPp, fmtTex, sigmoide } from "./logit-slides";

export { BETA0, BETA1, BETA2, arredondar, fmt, fmtPct, fmtPp, fmtTex, sigmoide };
...
/** O retorno de cada alternativa pode ter trechos em TeX entre cifrões (ComTex). */
...
export const ALTERNATIVAS: Alternativa[] = [
  ...
  { id: "B", texto: "O escore em log odds aumenta 0,7453, mantendo o atraso constante.", correta: true,
    ...
    feedback: String.raw`+10 pp correspondem a uma unidade da variável. Portanto, $\Delta z = 0{,}7453 \times 1$.` },
  { id: "C", texto: "A PD é multiplicada por 0,7453.", correta: false,
    tituloFeedback: "O coeficiente não é um multiplicador da PD",
    feedback: String.raw`O multiplicador das odds é $\exp(0{,}7453)$, aproximadamente 2,11. A PD exige outra transformação.` },
];
...
export const SITUACAO = [
  ...
  { k: "Coeficiente da utilização", v: `β = ${fmt(BETA1, 4)}`, tex: String.raw`\beta = ${fmtTex(BETA1, 4)}`, n: "Por unidade de 10 pp", cor: "navy" as const },
] as { k: string; v: string; tex?: string; n: string; cor: "ambar" | "roxo" | "navy" }[];
...
export const SINTESE_DEPOIS = String.raw`$\beta$ soma no log odds. $\exp(\beta)$ multiplica as odds. A mudança na PD depende do ponto de partida.`;
...
/** As duas contas exibidas na demonstração, montadas a partir do caso e dos coeficientes, em texto (rótulo acessível) e em TeX. */
export function contas() {
  const d = demonstracao();
  return [
    { texto: `Δx = (${CASO.utilPara} − ${CASO.utilDe}) ÷ 10 = ${fmt(d.dx, 0)}`, tex: String.raw`\Delta x = (${CASO.utilPara} - ${CASO.utilDe}) \div 10 = ${fmtTex(d.dx, 0)}` },
    { texto: `Δz = β × ${fmt(d.dx, 0)} = ${fmt(d.dz, 4)}`, tex: String.raw`\Delta z = \beta \times ${fmtTex(d.dx, 0)} = ${fmtTex(d.dz, 4)}` },
  ];
}
